...
export class PublisherService {
  ...
  async findByName(names: Array<string>, libraryId: string): Promise<Array<Types.ObjectId>> {
    const existingResources = await this.publisherModel.find({name: {$in: names}, libraryId: new Types.ObjectId(libraryId)}).lean();
    ...
    if (missingNames.length > 0) {
      const newResources = await this.publisherModel.insertMany(
