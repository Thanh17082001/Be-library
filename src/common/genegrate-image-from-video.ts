...
export const generateImageFromVideo = async (videoPathFile: string, outputImagevideoFolder: string = 'publication', time: string = '00:00:14'): Promise<{path: string; sizeMB: number}> => {
    ...
    ffmpeg(videoPath)
      ...
      .on('end', async () => {
        try {
          ...
          resolve({path: cutPath(outputImagePath), sizeMB});
        } catch (error) {
          console.log(error);
        }
      })
